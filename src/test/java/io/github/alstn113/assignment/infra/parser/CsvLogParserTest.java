...
class CsvLogParserTest extends AbstractIntegrationTest {
    ...
    void parseCsvSuccessfully() {
        // given
        URL resource = getClass().getClassLoader().getResource("test_logs.csv");
        File file = new File(resource.getFile());

        // when
        LogParser.ParseResult result = csvLogParser.parse(file);

        // then
        assertThat(result.logEntries()).hasSize(2);
        assertThat(result.parsingErrors().errorCount()).isZero();

        LogEntry first = result.logEntries().get(0);
